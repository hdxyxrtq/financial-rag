...
from src.utils import _AUTH_KEYWORDS, _QUOTA_KEYWORDS, async_call_with_retry, call_with_retry
...
class ZhipuReranker:
    ...
    def rerank(
        ...
        try:
            data = call_with_retry(
                ...
                non_retriable_types=(RerankAuthError, RerankQuotaError),
            )
            ...
            return [RerankResult(index=i, relevance_score=1.0, content=doc) for i, doc in enumerate(documents[:top_n])]
    ...
    async def arerank(
        ...
        try:
            data = await async_call_with_retry(
                do_call,
                _classify_rerank_error,
                non_retriable_types=(RerankAuthError, RerankQuotaError),
            )
        ...
        logger.info("异步 Rerank 完成：输入 %d 文档，返回 %d 条", len(documents), len(results))
        return results
